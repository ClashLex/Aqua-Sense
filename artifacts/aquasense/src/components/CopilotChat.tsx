      ...
      <AnimatePresence>
        {open && (
          ...
          >
            ...
            {!configured ? (
              <div className="flex-1 flex flex-col justify-center px-4 text-center space-y-3">
                <Bot className="w-8 h-8 mx-auto text-[#2563eb]" />
                <p className="text-[12px] font-black uppercase" style={{ color: "var(--app-text-1)" }}>
                  AI Key Required
                </p>
                <p className="text-[10px] font-bold" style={{ color: "var(--app-text-3)" }}>
                  Configure your OpenRouter API key to activate the assistant:
                </p>
                <div className="flex gap-1.5">
                  <input
                    ...
                    className="flex-1 bg-transparent text-[10px] px-2.5 py-1.5 rounded-md outline-none border-2 border-black dark:border-white font-semibold"
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 rounded-md text-[10px] font-extrabold uppercase border-2 border-black dark:border-white bg-[#2563eb] text-white cursor-pointer"
                  >
                    Save
                  </button>
                </div>
              ...
              <>
                ...
                <div ref={scrollRef} className="flex-1 overflow-y-auto px-3 py-3 space-y-3.5">
                  ...
                  {keyError && (
                    <div
                      className="rounded-md border-2 border-black dark:border-white p-2.5 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]"
                      style={{ background: "var(--app-surface-2)" }}
                    >
                      <p className="text-[10px] font-black uppercase mb-1" style={{ color: "var(--app-text-1)" }}>
                        AI Key Required on Cloudflare
                      </p>
                      <p className="text-[9px] mb-2 font-semibold" style={{ color: "var(--app-text-3)" }}>
                        Direct browser connection requires a custom OpenRouter key:
                      </p>
                      <div className="flex gap-1">
                        <input
                          ...
                          value={customKeyInput}
                          onChange={(e) => setCustomKeyInput(e.target.value)}
                          className="flex-1 bg-transparent text-[10px] px-2 py-1 rounded-md outline-none border border-black dark:border-white font-semibold"
                          style={{ color: "var(--app-text-1)" }}
                        />
                        <button
                          onClick={() => {
                          ...
                          className="px-2 py-1 rounded-md text-[9px] font-extrabold uppercase border border-black dark:border-white bg-[#2563eb] text-white cursor-pointer"
                        >
                          Save
                        </button>
                      </div>
